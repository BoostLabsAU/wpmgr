 ...
 *
 * Original implementation. The CONCEPT (structure-hash dedup) is standard; the
 * exact recipe here is ours.
 *
...
final class StructureHash
{
    ...
    public static function compute(string $html, array $includeSelectors = []): string
    {
        ...
        // 2. class tokens (digits stripped so post-123 == post-456 collapse).
        if (preg_match_all('/\sclass=["\']([^"\']*)["\']/i', $html, $m)) {
            foreach ($m[1] as $classAttr) {
                foreach (preg_split('/\s+/', trim($classAttr)) ?: [] as $cls) {
                    ...
                    $tokens['c:' . preg_replace('/\d+/', '', $cls)] = true;
                }
            }
        }

        // 3. id tokens (digits stripped).
        if (preg_match_all('/\sid=["\']([^"\']*)["\']/i', $html, $m)) {
            foreach ($m[1] as $id) {
                $id = trim($id);
                if ($id !== '') {
                    $tokens['i:' . preg_replace('/\d+/', '', $id)] = true;
                }
            }
        }
