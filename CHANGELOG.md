...
- Incremental backups are now bulletproof. The archive-delta engine (ADR-051, agent 0.21.6) replaces the previous per-file chunk scanner. An increment now runs the same proven archive pipeline as a full backup: compare the live file tree against the parent snapshot's file list by size and modified time, pack only changed and new files into standard part archives, and stream deletions to a tombstone sidecar on disk. The state the agent must carry across requests dropped from thousands of per-file records to roughly 25 part names, which is the same tiny cursor a full backup uses and the root cause of the previous 0-files silent data-loss bug. Increments are smaller because unchanged files are skipped entirely; restore overlays each generation in order with newest-wins extraction and then applies tombstone deletes so any point in the chain restores correctly. The database is dumped in full on every backup. (commit 642270a)
...
- Backup restore: point-in-time version picker and chain view. Incremental backups now render as a single expandable row grouping the base backup and all its increments. When restoring, pick the exact snapshot to roll back to; files and database both restore to the chosen point in the chain.
...
- Media Optimizer: the scale-to-zero encoder now shuts down gracefully, preventing in-progress optimize jobs from stalling on encoder cold-start races. Cancelling an optimize job also cancels its background encode job so no orphaned work remains.
- Unused Image Cleaner: hardened the reference scan to close false-negative gaps: block-editor image IDs, SEO plugin image meta fields, options-page image IDs, and large media libraries (pagination) are all now covered. The optimizer's own bookkeeping metadata is excluded so optimizer-managed images are never incorrectly flagged as unused.
- Backups: routes-contract test added for the restore-run and schedule-run endpoints; both endpoints are now documented in the API spec.
